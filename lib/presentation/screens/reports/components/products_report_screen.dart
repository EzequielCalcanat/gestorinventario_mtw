...
class _ProductsReportScreenState extends State<ProductsReportScreen> {
  ...
  Future<void> _fetchProducts() async {
    ...
    products = await ProductRepository.getTopSellingProducts(startDate!, endDate!);
          ...
          isLoading
            ...
            child: Scrollbar(
              child: products.isEmpty
                  ? const Center(child: Text("No hay productos en este rango."))
                  : ListView.builder(
                itemCount: products.length,
                itemBuilder: (context, index) {
                  final product = products[index];
                  final quantity = product['total_quantity'] ?? 0;
                  final totalSales = (product['total_sales'] as num?) ?? 0.0;
  ...
  Widget _buildProductsChart() {
    ...
    for (var product in products.take(10)) {
      chartData[product['product_name']] = (product['total_quantity'] as num?)?.toDouble() ?? 0.0;
    }
